...
struct rfkill_rk_data {
	...
	struct wakeup_source *bt_irq_ws;
...
static irqreturn_t rfkill_rk_wake_host_irq(int irq, void *dev)
{
	...
	__pm_wakeup_event(rfkill->bt_irq_ws,
			  BT_IRQ_WAKELOCK_TIMEOUT);
				...
				const char *name)
{
	if (gpio->desc) {
		...
		/* GPIO descriptor acquired via devm_ functions doesn't need explicit request */
		return 0;
	}
...
static int rfkill_rk_setup_wake_irq(struct rfkill_rk_data *rfkill, int flag)
{
	...
	if (irq->gpio.desc) {
		...
		irq->irq = gpiod_to_irq(irq->gpio.desc);
...
fail3:
	free_irq(irq->irq, rfkill);
fail2:
	/* GPIO is managed by devm_, no need to free */
fail1:
	return ret;
}
					       ...
					       bool sleep)
{
	...
	if (wake->desc)
		gpiod_direction_output(wake->desc, sleep ? !wake->enable : wake->enable);
#else
	if (!sleep) {
		...
		if (wake->desc) {
			gpiod_direction_output(wake->desc, wake->enable);
			usleep_range(10, 20);
			gpiod_direction_output(wake->desc, !wake->enable);
		}
	}
...
static int rfkill_rk_set_power(void *data, bool blocked)
{
	...
	if (!blocked) {
		...
		if (wake_host->desc) {
			...
			gpiod_direction_output(wake_host->desc, 1);
			msleep(20);
		}

		if (poweron->desc) {
			if (gpiod_get_value(poweron->desc) == !poweron->enable) {
				gpiod_direction_output(poweron->desc,
						      !poweron->enable);
				msleep(20);
				gpiod_direction_output(poweron->desc,
						      poweron->enable);
		...
		if (reset->desc) {
			if (gpiod_get_value(reset->desc) == !reset->enable) {
				gpiod_direction_output(reset->desc,
						      !reset->enable);
				msleep(20);
				gpiod_direction_output(reset->desc, reset->enable);
			}
		}

		if (wake_host->desc) {
			LOG("%s: set bt wake_host input!\n", __func__);
			gpiod_direction_input(wake_host->desc);
		}

		if (pinctrl && rts->desc) {
			pinctrl_select_state(pinctrl, rts->gpio_state);
			LOG("ENABLE UART_RTS\n");
			gpiod_direction_output(rts->desc, rts->enable);
			msleep(100);
...
static int rfkill_rk_pm_prepare(struct device *dev)
{
	...
	if (rfkill->pdata->pinctrl && rts->desc) {
	...
	if (wake_host_irq->gpio.desc && bt_power_state) {
				       ...
				       struct rfkill_rk_platform_data *data)
{
	...
	desc = devm_gpiod_get_optional(dev, "uart-rts", GPIOD_ASIS);
	if (!IS_ERR_OR_NULL(desc)) {
		data->rts_gpio.desc = desc;
		data->rts_gpio.enable = (gpiod_is_active_low(desc)) ? 0 : 1;
		LOG("%s: get property: uart_rts_gpios acquired.\n", __func__);
		...
		data->pinctrl = NULL;
		data->rts_gpio.desc = NULL;
		LOG("%s: uart_rts_gpios is no-in-use.\n", __func__);
	}

	desc = devm_gpiod_get_optional(dev, "bt-power", GPIOD_ASIS);
	if (!IS_ERR_OR_NULL(desc)) {
		data->poweron_gpio.desc = desc;
		data->poweron_gpio.enable = (gpiod_is_active_low(desc)) ? 0 : 1;
		LOG("%s: get property: BT,power_gpio acquired.\n", __func__);
	} else {
		data->poweron_gpio.desc = NULL;
	}
	
	desc = devm_gpiod_get_optional(dev, "bt-reset", GPIOD_ASIS);
	if (!IS_ERR_OR_NULL(desc)) {
		data->reset_gpio.desc = desc;
		data->reset_gpio.enable = (gpiod_is_active_low(desc)) ? 0 : 1;
		LOG("%s: get property: BT,reset_gpio acquired.\n", __func__);
	} else {
		data->reset_gpio.desc = NULL;
	}
	
	desc = devm_gpiod_get_optional(dev, "bt-wake", GPIOD_ASIS);
	if (!IS_ERR_OR_NULL(desc)) {
		data->wake_gpio.desc = desc;
		data->wake_gpio.enable = (gpiod_is_active_low(desc)) ? 0 : 1;
		LOG("%s: get property: BT,wake_gpio acquired.\n", __func__);
	} else {
		data->wake_gpio.desc = NULL;
	}
	
	desc = devm_gpiod_get_optional(dev, "bt-wake-host", GPIOD_IN);
	if (!IS_ERR_OR_NULL(desc)) {
		data->wake_host_irq.gpio.desc = desc;
		data->wake_host_irq.gpio.enable = (gpiod_is_active_low(desc)) ? 0 : 1;
		LOG("%s: get property: BT,wake_host_irq acquired.\n", __func__);
	} else {
		data->wake_host_irq.gpio.desc = NULL;
	}
...
static int rfkill_rk_probe(struct platform_device *pdev)
{
	...
	rfkill->bt_irq_ws = wakeup_source_register(&pdev->dev,
		       "rfkill_rk_irq_wl");
	if (!rfkill->bt_irq_ws)
		goto fail_gpio;
	...
	//rfkill_rk_set_power(rfkill, BT_BLOCKED);
	// bt turn off power
	if (pdata->poweron_gpio.desc) {
		gpiod_direction_output(pdata->poweron_gpio.desc,
				      !pdata->poweron_gpio.enable);
	}
	if (pdata->reset_gpio.desc) {
		gpiod_direction_output(pdata->reset_gpio.desc,
...
fail_setup_wake_irq:
	wakeup_source_unregister(rfkill->bt_irq_ws);
fail_gpio:
...
static void rfkill_rk_remove(struct platform_device *pdev)
{
	...
	// GPIO descriptors acquired via devm_ are automatically freed
	
	if (rfkill->pdata->wake_host_irq.gpio.desc) {
...
static int __init rfkill_rk_init(void)
{
	...
	/* Explicitly instantiate bluetooth platform device from DT */
	np = of_find_compatible_node(NULL, NULL, "bluetooth-platdata");
	if (np) {
