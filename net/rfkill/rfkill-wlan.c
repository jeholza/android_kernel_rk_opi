...
struct rfkill_wlan_data {
	...
	struct wakeup_source *wlan_irq_ws;
};
...
int rfkill_set_wifi_bt_power(int on)
{
	...
	if (vbat->desc) {
		if (on)
			gpiod_direction_output(vbat->desc, vbat->enable);
		else
			gpiod_direction_output(vbat->desc, !(vbat->enable));
	}
...
int rockchip_wifi_power(int on)
{
	...
	if (mrfkill->pdata->mregulator.power_ctrl_by_pmu) {
		...
		if (on) {
			...
			if (poweron->desc) {
				gpiod_direction_output(poweron->desc, poweron->enable);
				msleep(100);
			}
...
int rockchip_wifi_get_oob_irq(void)
{
	...
	if (wifi_int_irq->desc) {
		return gpiod_to_irq(wifi_int_irq->desc);
	} else {
				  ...
				  struct rksdmmc_gpio_wifi_moudle *data)
{
	...
	if (of_find_property(node, "power_ctrl_by_pmu", NULL)) {
		...
		desc = devm_gpiod_get_optional(dev, "wifi-poweren", GPIOD_ASIS);
		if (!IS_ERR_OR_NULL(desc)) {
			data->power_n.desc = desc;
			data->power_n.enable = 
				(gpiod_is_active_low(desc)) ? 0 : 1;
			LOG("%s: WIFI,poweren_gpio acquired, active_low = %d.\n",
			    __func__, gpiod_is_active_low(desc));
		} else {
			data->power_n.desc = NULL;
		}
		
		desc = devm_gpiod_get_optional(dev, "wifi-vbat", GPIOD_ASIS);
		if (!IS_ERR_OR_NULL(desc)) {
			data->vbat_n.desc = desc;
			data->vbat_n.enable = 
				(gpiod_is_active_low(desc)) ? 0 : 1;
			LOG("%s: WIFI,vbat_gpio acquired, active_low = %d.\n",
			    __func__, gpiod_is_active_low(desc));
		} else {
			data->vbat_n.desc = NULL;
		}
		
		desc = devm_gpiod_get_optional(dev, "wifi-reset", GPIOD_ASIS);
		if (!IS_ERR_OR_NULL(desc)) {
			data->reset_n.desc = desc;
			data->reset_n.enable = 
				(gpiod_is_active_low(desc)) ? 0 : 1;
			LOG("%s: WIFI,reset_gpio acquired, active_low = %d.\n",
			    __func__, gpiod_is_active_low(desc));
		} else {
			data->reset_n.desc = NULL;
		}
		
		desc = devm_gpiod_get_optional(dev, "wifi-host-wake", GPIOD_IN);
		if (!IS_ERR_OR_NULL(desc)) {
			data->wifi_int_b.desc = desc;
			data->wifi_int_b.enable = 
				(gpiod_is_active_low(desc)) ? 0 : 1;
			LOG("%s: WIFI,host_wake_irq acquired, active_low = %d.\n",
			    __func__, gpiod_is_active_low(desc));
		} else {
			data->wifi_int_b.desc = NULL;
		}
	}
...
static int rfkill_wlan_probe(struct platform_device *pdev)
{
	...
	rfkill->wlan_irq_ws = wakeup_source_register(&pdev->dev,
		       "rfkill_wlan_wake");
	if (!rfkill->wlan_irq_ws)
		goto fail_alloc;
	rfkill_set_wifi_bt_power(1);

#ifdef CONFIG_SDIO_KEEPALIVE
	if (pdata->power_n.desc)
		gpiod_direction_output(pdata->power_n.desc, pdata->power_n.enable);
#endif
...
static void rfkill_wlan_remove(struct platform_device *pdev)
{
	...
	wakeup_source_unregister(rfkill->wlan_irq_ws);
...
int __init rfkill_wlan_init(void)
{
	...
	/* Explicitly instantiate wlan platform device from DT */
