 ...
 *
 * Ported from kernel-5.10 (Android_13) for use with rockchip,hdmi DTS binding.
 */
...
static int rk_dailink_init(struct snd_soc_pcm_runtime *rtd)
{
	...
	widgets = devm_kcalloc(dev, 1,
			       sizeof(*widgets), GFP_KERNEL);
	if (!widgets)
		return -ENOMEM;

	ret = rk_hdmi_fill_widget_info(dev, widgets,
				...
				char *prefix)
{
	...
	snd_soc_daifmt_parse_clock_provider_raw(node, prefix,
...
static int rk_hdmi_probe(struct platform_device *pdev)
{
	...
	/* codec is first available codec (hdmi0 or hdmi1) */
	rk_data->dai.dai_fmt = rk_hdmi_parse_daifmt(np, cpu_np,
						    codecs[0].of_node, "rockchip,");
